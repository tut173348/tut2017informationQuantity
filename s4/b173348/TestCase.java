...
public class TestCase {
  public static void main(String[] args) {
    try {
      ...
      freq = myObject.frequency();
      if(freq == 0) {
        System.out.println("SPACE is not set or SPACE's length is zero     [OK]");
      }else {
      ...
      freq = myObject.frequency();
      if(freq == -1) {
        System.out.println("TARGET is not set or TARGET's length is zero     [OK]");
      }else {
      ...
      //Testcase SPACE = TARGET
      System.out.println("\n----------Testcase 9----------");
      myObject.setSpace("Hi Ho Hi Ho".getBytes());
      myObject.setTarget("Hi Ho Hi Ho".getBytes());
      freq = myObject.frequency();
      System.out.print("\"Hi Ho Hi Ho\" in \"Hi Ho Hi Ho\" appears "+freq+" times. ");
      if(1 == freq) { System.out.println("[OK]"); } else {System.out.println("WRONG"); }
    }
    catch(Exception e) {
    ...
    try {
      ...
      value = myObject.estimation();
      if(value == Double.MAX_VALUE) {
        System.out.println("TARGET is not included in SPACE     [OK]");
        //In this case, value1 is Infinity
      }else {
